...
class Day07 {
    ...
    private boolean maths(long target, long current, Long[] nums, boolean concatenate) {
        ...
        long combinedNum = Long.parseLong(String.format("%d%d", current, nums[0]));
        if (nums.length == 1) {
    ...
    private void solve() {
        ...
        for (ArrayList<Long> equation : equations) {
            ...
            if (equation.size() <= 3) {
                if (Long.parseLong(String.format("%d%d", currentList[1], currentList[2])) == currentList[0]) {
                    partTwoSum += currentList[0];
                    continue;
                }
            } else if (maths(currentList[0], Long.parseLong(String.format("%d%d", currentList[1], currentList[2])),
